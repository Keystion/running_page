...
const LIGHTS_ON = true; // 默认开灯:否则底图图层被隐藏,只剩轨迹(黑底)
...
const FULL_MARATHON_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT}（🐎🪢🌲）` : 'Full Marathon';
const HALF_MARATHON_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT}（🐎🪢🌲/2）` : 'Half Marathon';
const MORNING_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT} 🕖` : 'Morning Run';
const MIDDAY_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT} 🌞` : 'Midday Run';
const AFTERNOON_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT} 🌇` : 'Afternoon Run';
const EVENING_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT} 🌅` : 'Evening Run';
const NIGHT_RUN_TITLE = IS_CHINESE ? `${RUN_TEXT} 🌌` : 'Night Run';
const RUN_GENERIC_TITLE = IS_CHINESE ? `${RUN_TEXT}` : 'Run';
const RUN_TRAIL_TITLE = IS_CHINESE ? `${RUN_TEXT}` : 'Trail Run';
const RUN_TREADMILL_TITLE = IS_CHINESE ? `${RUN_TEXT}` : 'Treadmill Run';
const HIKING_TITLE = IS_CHINESE ? `${HIKE_TEXT}` : 'Hiking';
const CYCLING_TITLE = IS_CHINESE ? `${RIDE_TEXT}` : 'Cycling';
const SKIING_TITLE = IS_CHINESE ? `${SKIING_TEXT}` : 'Skiing';
const WALKING_TITLE = IS_CHINESE ? `${HIKE_TEXT}` : 'Walking';
const SWIMMING_TITLE = IS_CHINESE ? `${SWIM_TEXT}` : 'Swimming';
...
// mapbox 官方 style(vector v8),与 MapboxLanguage 中文标签兼容
...
// mapcn: empty
// mapbox 厂商:鉴权实际用上面的 MAPBOX_TOKEN(即 VITE_MAPBOX_TOKEN);此处复用同一个 token,
// 仅为通过下方「需要 access token」的配置校验(避免控制台报错)。
